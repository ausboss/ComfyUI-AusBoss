...
def register_lama_model_folder() -> None:
    ...
    folder_paths.add_model_folder_path(MODEL_FOLDER, model_directory)
    _paths, extensions = folder_paths.folder_names_and_paths[MODEL_FOLDER]
...
def list_lama_models() -> list[str]:
    ...
    return folder_paths.get_filename_list(MODEL_FOLDER) or [DEFAULT_MODEL_NAME]
...
def resolve_lama_model(model_name: str) -> Path:
    ...
    raise FileNotFoundError(
        ...
        f"checkpoint at ComfyUI/models/lama/{DEFAULT_MODEL_NAME}, then restart "
        "ComfyUI."
    )
...
@lru_cache(maxsize=2)
def _load_lama_model(model_path: str, device_name: str, modified_ns: int):
    del modified_ns  # Part of the cache key so replacing a checkpoint reloads it.
...
def load_lama_model(model_path: str, device_name: str):
    modified_ns = Path(model_path).stat().st_mtime_ns
    return _load_lama_model(model_path, device_name, modified_ns)
...
def inpaint_with_model(
    ...
    for index in range(image_count):
        ...
        soft_mask = normalized_masks[index].unsqueeze(0).unsqueeze(0)
        if not torch.any(soft_mask > 0.0):
            outputs.append(source.to(dtype=images.dtype))
            continue
        ...
        if (
            ...
            or generated.shape[-2:] != rgb.shape[-2:]
        ):
            raise RuntimeError(
                "The selected LaMa model did not return a matching BCHW RGB batch."
            )
        if not torch.isfinite(generated).all():
            raise RuntimeError("The selected LaMa model returned non-finite values.")

        generated = generated[:, :3, :height, :width]
...
def run_lama_inpaint(
    ...
    model = load_lama_model(str(model_path), str(device))

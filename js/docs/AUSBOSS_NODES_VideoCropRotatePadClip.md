...
- **Uploads**: Pick an existing input video, click **Upload**, or drop a video file
  ...
  are now driven by this compact card.
- **Local path**: Read a file on the ComfyUI server in place without copying it.
  Queued workflows always read it; the editor's live preview of paths outside ComfyUI's
  own folders requires starting ComfyUI with `AUSBOSS_TRANSFORM_LOCAL_PREVIEW=1`.
- **IN / OUT**: Drag the trim handles on the node or in the editor, or scrub/type exact
  seconds below them. **Full clip** resets the window. These drive `start_seconds` /
  `end_seconds`; an end of 0 means the end of the source. While a handle moves, the
  stage peeks at the frame under it and returns to the playhead on release; the scrub
  bar and the saved preview position never move with a trim.
...
- **frame_index / frame_time**: The editor's separate preview position, not trim points.
  Preview scrubbing does not invalidate the queued clip; the position is still saved in the workflow.
...
it; tap the lit chip to clear the bands again.
